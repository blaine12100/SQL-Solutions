...
With base_select as (SELECT distinct(company_id) as duplicate_companies FROM job_listings group by company_id, title, description having count(*) > 1)

select count(*) as duplicate_companies from base_select